...
void bookRoom() {
    ...
    struct customer c;
    struct room r;
    FILE *roomFile, *tempRoomFile, *bookingFile;
    ...
    // Create a temporary room file for updated room data
    tempRoomFile = fopen("TempRoomData.txt", "w");
    if (tempRoomFile == NULL) {
        printf("Error creating temporary room data file.\n");
        fclose(roomFile);
        return;
    }
    ...
    while (fscanf(roomFile, "Room Number: %d\n", &r.roomNumber) != EOF) {
        ...
        if (r.roomNumber == roomNumber) {
            ...
            strcpy(r.availability, "No");
        }

        // Write room data (updated or unchanged) to the temporary file
        fprintf(tempRoomFile, "Room Number: %d\n", r.roomNumber);
        fprintf(tempRoomFile, "Room Type: %s\n", r.roomType);
        fprintf(tempRoomFile, "Price: %.2f\n", r.price);
        fprintf(tempRoomFile, "Availability: %s\n", r.availability);
        fprintf(tempRoomFile, "-------------------------\n");
    }
    ...
    // Replace the old room file with the updated one
    remove("RoomData.txt");
    rename("TempRoomData.txt", "RoomData.txt");
}
...
void viewBookings(){
    ...
    printf("\nYour bookings:\n\n");
    while (fgets(line, sizeof(line), bookingFile)) {
        if (scanf(line, "Customer ID: %d", &c.customerID) == 1 && c.customerID == customerID) {
            printf("Customer ID: %d\n", c.customerID);
            fgets(line, sizeof(line), bookingFile);
            scanf(line, "Customer Name: %s", c.customerName);
            printf("Customer Name: %s\n", c.customerName);
            fgets(line, sizeof(line), bookingFile);
            scanf(line, "Room Number: %d", &c.roomNumber);
            printf("Room Number: %d\n", c.roomNumber);
            fgets(line, sizeof(line), bookingFile);
            scanf(line, "Booking Date: %s", c.bookingDate);
